...
/*******************************************************************************
 * Application functions (from AD5940Main.c)
 ******************************************************************************/

/**
 * @brief Display impedance measurement results
 */
static int32_t ImpedanceShowResult(uint32_t *pData, uint32_t DataCount)
{
    ...
    LOG_INF("Freq: %.2f Hz", freq);
    
    for (int i = 0; i < DataCount; i++) {
        LOG_INF("  RzMag: %.2f Ohm, RzPhase: %.2f deg",
                pImp[i].Magnitude, 
...
static void AD5940ImpedanceStructInit(void)
{
    ...
    pImpedanceCfg->SweepCfg.SweepPoints = 101;      /* 101 points */
