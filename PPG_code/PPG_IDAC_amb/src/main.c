...
static inline led_state_t get_led_state(void)
{
    ...
    bool red = (p0 & (1u << 9))  != 0;   // P0.9
    bool ir  = (p0 & (1u << 10)) != 0;   // P0.10
...
static void ppi_route_all(void)
{
    ...
    /* -------- LEDs (same as your working design) -------- */
    /* CC0: Green ON */
...
static void sequencer_start(void)
{
    ...
    //nrf_gpio_pin_clear(GRN_EN_PIN);
    nrf_gpio_pin_set(GRN_EN_PIN); //start inverted
    nrf_gpio_pin_clear(RED_EN_PIN);
    nrf_gpio_pin_clear(IR_EN_PIN);
...
/* ======================= IDAC HELPER FUNCTION ====================== */
int set_PWM(uint32_t dark_avg_mv, int32_t *mv_control){

    //===== CALCULATE CHANGE IN INJECTED CURRENT AND PWM VOLTAGE =====
    ...
    int32_t delta_Vcontrol_mV = Vdrop_nV/500000;

    //printk("Delta Control=%ld mV \n",(long)delta_Vcontrol_mV);

    //================================================================

    //==================== DETERMINE NEW V_CONTROL ===================
    //clamp at limits
    *mv_control += delta_Vcontrol_mV;

    if (*mv_control > (int32_t)V_SUPPLY_mV){
        *mv_control = V_SUPPLY_mV;
    } else if (*mv_control < 0){
        *mv_control = 0U;
    } 
    //================================================================
    ...
    /* Testing prints */
    /*
    printk("Current CTRL=%ld mV -> PWM=%u ns | AVG SAMPLE=%ld mV | DIFF=%ld mV\n",
    (long)*mv_control, pulse_ns, (long)dark_avg_mv, (long)(Vdiff_nV/1000000));
    printk("DELTA_CONTROL=%ld mV | I_inj=%ld nA \n",
    (long)delta_Vcontrol_mV, (long)(Iinj_pA/1000));
    */
}
/* =================================================================== */
...
int main(void)
{
    ...
    uint32_t T_pwm_ns = 1000000000UL / F_PWM_HZ; //calculate period of PWM signal is ns
    (void)pwm_set_dt(&pwm, T_pwm_ns, 0); //initialize PWM signal at 0V

    /* GPIOTE outputs (LEDs + S_EN) */
    gpiote_toggle_init(CH_GRN, GRN_EN_PIN, true); // invert green
    gpiote_toggle_init(CH_RED, RED_EN_PIN, false);
    gpiote_toggle_init(CH_IR,  IR_EN_PIN, false);
    ...
    while (1) {
        ...
        if (st != last_st){
            ...
            if (st == 0) {
                ...
                //ensure change has settled before accepting samples
                if (IDAC_time_acc_us > IDAC_tau_us){
                    IDAC_sample_acc_mv += st_reading[3];
                    IDAC_sample_count++;
                }
                if (IDAC_time_acc_us >= T_IDAC_us){
                    set_PWM(IDAC_sample_acc_mv/IDAC_sample_count, &mv_control);
                    //IDAC cycle complete reset all
                    IDAC_time_acc_us = 0;
                    IDAC_sample_acc_mv = 0;
                    IDAC_sample_count = 0;
                }
                cycle_dt = 0;
            }
