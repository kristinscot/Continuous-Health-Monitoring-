...
int main(void)
{
    ...
    while (1) {
        for (int state=0; state<4; state++) {
            ...
            // TODO - I can move this into a function and/or make it simpler by removing the for loop since only one channel
            for (size_t i = 0; i < ARRAY_SIZE(adc_channels); i++) {
                int32_t mv;
                ...
                if (adc_raw_to_millivolts_dt(&adc_channels[i], &mv) == 0) {
                    printf("mv=%d\n", mv);
                } else {
            ...
            k_usleep(stateDuration[state]); //Temporary for testing
