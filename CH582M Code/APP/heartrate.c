...
// --- Button config ---
#define BUTTON_PORTB_PIN           GPIO_Pin_15   // PB15
#define BUTTON_SAMPLE_PERIOD_MS    50            // how often to poll
...
#define DEFAULT_HEARTRATE_PERIOD   BUTTON_SAMPLE_PERIOD_MS
...
static uint8_t lastBtn = 0xFF; // "unknown" to force first notify
...
static uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "Heart Cate";
...
void HeartRate_Init()
{
    ...
    // Configure PB15 as input pull-up for button
    GPIOB_ModeCfg(BUTTON_PORTB_PIN, GPIO_ModeIN_PU);
...
static void heartRateMeasNotify(void)
{
    // Read active-low button on PB15
    uint8_t raw = GPIOB_ReadPortPin(BUTTON_PORTB_PIN) ? 1 : 0;  // 1=released, 0=pressed
    uint8_t btn = (raw == 0) ? 1 : 0; // map to 1=pressed, 0=released

    // Only notify on change (comment out this if you want every tick)
    if(btn == lastBtn) {
        return;
    }
    lastBtn = btn;

    // Allocate a tiny HRM payload: [flags=0][value=btn]
    heartRateMeas.pValue = GATT_bm_alloc(gapConnHandle, ATT_HANDLE_VALUE_NOTI, 2, NULL, 0);
    if(heartRateMeas.pValue != NULL)
    {
        uint8_t *p = heartRateMeas.pValue;
        *p++ = 0x00;      // HRM flags = 0 (we're repurposing the field)
        *p++ = btn;       // "BPM" byte becomes button state (0/1)
